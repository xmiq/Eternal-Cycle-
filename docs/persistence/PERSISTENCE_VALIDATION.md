...
FR-011 turn validation additionally confirms that the configured `DIRECT` or `MCP` target was resolved before state-changing play, required owner reads preceded resolution, the Affected Set was explicitly determined, every non-empty set used one owner-routed transaction, the configured authority proved the expected change, the resulting Save Point activated only after required verification, stale Context Packets were invalidated after activation, and relevant Turn N state can be retrieved on Turn N+1. Direct completion uses adapter evidence; MCP completion requires a validated Persistence Receipt. A non-empty Affected Set whose expected canonical state remains unchanged is a validation failure. Repository validation executes the implementation-neutral completion-gate harness; runtime hosts must also execute these cases against their configured mode.
