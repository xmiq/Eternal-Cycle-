...
The active Save Index and Campaign Configuration identify the persisted `DIRECT` or `MANAGED` strategy and its canonical authority. That selection is reused unless an explicit validated migration changes it, and its target must be resolved before state-changing play. A Direct local commit proves completion only for a local-authoritative campaign; Direct cloud authority additionally requires remote synchronization and read-back. Managed authority requires validated service completion evidence and exposes no client-classified local/cloud topology. Player-visible `💾`, `☁️💾`, `⏳`, and `⚠️` markers report the resulting state and never replace the underlying evidence.
...
| Persistence transaction, deployment, backup, and Read-Back Validation procedures | selected Direct Adapter Chain or Managed Data Service |
...
A Direct Adapter Chain may compose distinct responsibilities. For example, the [SQLite Database Format Adapter](adapters/SQLITE_DATABASE_FORMAT_ADAPTER.md) may own database transactions and integrity while the [Google Drive Remote Storage Adapter](adapters/GOOGLE_DRIVE_REMOTE_STORAGE_ADAPTER.md) owns canonical remote identity, deployment, backup, and remote read-back. Campaign Configuration selects `DIRECT` and orders the chain; neither adapter adjudicates gameplay. In `MANAGED`, the configured service owns its backend and returns semantic persistence evidence through its selected interface without exposing a client-side adapter chain.
