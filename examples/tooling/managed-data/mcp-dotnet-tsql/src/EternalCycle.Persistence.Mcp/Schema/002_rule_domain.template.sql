...
-- Render only through trusted domain-namespace configuration. The reference
-- mapping is ec_domain; alternatives remain physical deployment mappings.
...
CREATE TABLE {{schema}}.rule_releases (
...
CREATE TABLE {{schema}}.rule_chunks (
    rule_release_id nvarchar(128) NOT NULL,
...
CREATE TABLE {{schema}}.rule_chunk_selectors (
    rule_release_id nvarchar(128) NOT NULL,
    chunk_id nvarchar(256) NOT NULL,
    ...
    CONSTRAINT FK_ec_domain_rule_chunk_selectors_chunk FOREIGN KEY (rule_release_id, chunk_id)
        REFERENCES {{schema}}.rule_chunks(rule_release_id, chunk_id),
...
CREATE INDEX IX_ec_domain_rule_chunk_selector_lookup
    ON {{schema}}.rule_chunk_selectors (rule_release_id, selector_type, selector_value, chunk_id);
GO

CREATE TABLE {{schema}}.rule_dependencies (
    rule_release_id nvarchar(128) NOT NULL,
...
CREATE TABLE {{schema}}.active_rule_releases (
    ruleset_id nvarchar(128) NOT NULL,
    rule_release_id nvarchar(128) NOT NULL,
...
CREATE TABLE {{schema}}.rule_update_checks (
