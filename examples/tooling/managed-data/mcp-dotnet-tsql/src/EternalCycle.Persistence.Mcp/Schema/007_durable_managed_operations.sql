...
-- Durable Managed Operations, progressive Rule Source preparation, and
-- prerelease display provenance. Canonical rule identity remains source_identity.
IF OBJECT_ID(N'[ec_domain].managed_operations', N'U') IS NULL
BEGIN
    CREATE TABLE [ec_domain].managed_operations (
    ...
    CREATE INDEX IX_ec_domain_managed_operations_dispatch
        ON [ec_domain].managed_operations (operation_state, created_at, operation_id);

    CREATE INDEX IX_ec_domain_managed_operations_deduplication
        ON [ec_domain].managed_operations (deduplication_key, operation_state, updated_at DESC);

    CREATE INDEX IX_ec_domain_managed_operations_recent
        ON [ec_domain].managed_operations (operation_kind, updated_at DESC);
END;
GO

IF OBJECT_ID(N'[ec_domain].rule_source_preparation', N'U') IS NULL
BEGIN
    CREATE TABLE [ec_domain].rule_source_preparation (
    ...
    CREATE INDEX IX_ec_domain_rule_source_preparation_dispatch
        ON [ec_domain].rule_source_preparation (
...
-- Existing validated releases remain usable after this additive migration. Their
-- stored chunks are already complete, so their sources begin Ready.
INSERT INTO [ec_domain].rule_source_preparation (
...
SELECT
    ...
    0,
    SYSUTCDATETIME()
FROM [ec_domain].rule_chunks AS chunks
WHERE NOT EXISTS (
    SELECT 1
    FROM [ec_domain].rule_source_preparation AS existing
...
IF COL_LENGTH(N'ec_domain.rule_releases', N'base_release') IS NULL
    ALTER TABLE [ec_domain].rule_releases ADD base_release nvarchar(64) NULL;
GO

IF COL_LENGTH(N'ec_domain.rule_releases', N'discovery_tag') IS NULL
    ALTER TABLE [ec_domain].rule_releases ADD discovery_tag nvarchar(128) NULL;
GO

IF COL_LENGTH(N'ec_domain.rule_releases', N'commits_since_base') IS NULL
    ALTER TABLE [ec_domain].rule_releases ADD commits_since_base int NULL;
GO

IF COL_LENGTH(N'ec_domain.rule_releases', N'display_version') IS NULL
    ALTER TABLE [ec_domain].rule_releases ADD display_version nvarchar(128) NULL;
GO
