...
public static class EternalCycleErrorRegistry
{
    ...
    private static IEnumerable<EternalCycleErrorDefinition> CreateEntries()
    {
        ...
        yield return Def("MANAGED_OPERATION_CANCELLED_UNEXPECTED", "Unexpected Managed Operation cancellation", "A parent cancellation other than the classified host-shutdown or operation-timeout paths interrupted durable work.", "ManagedOperation", "Error", "RetrySafe", false, false, "Inspect operation and Git-process diagnostics before retrying; do not infer the unknown parent source.");
        ...
        yield return Def("RULE_SOURCE_INCOMPATIBLE", "Rule Source incompatible", "The acquired source does not satisfy Eternal Cycle manifest or compiler compatibility.", "RuleSource", "Error", "NotRetrySafe", true, true, "Select or repair any technically compatible Rule Source.");
        yield return Def("RULE_SOURCE_NOT_CONFIGURED", "Rule Source not configured", "No Rule Source selection exists.", "RuleSource", "Error", "RetryAfterConfiguration", true, true, "Select the official default or another compatible Rule Source after database setup completes.");
        yield return Def("RULE_SOURCE_ACQUISITION_TIMEOUT", "Rule Source acquisition timeout", "The complete Rule Source acquisition exceeded its configured overall bound.", "RuleSource", "Error", "RetrySafe", true, false, "Inspect correlated process diagnostics and retry only after identifying the observed bottleneck.");
        ...
        yield return Def("RULE_SOURCE_OPERATION_TIMEOUT", "Legacy Rule Source operation timeout", "A source-control operation exceeded a configured bound in an older diagnostic contract.", "RuleSource", "Error", "RetrySafe", true, false, "Inspect correlated diagnostics and distinguish overall acquisition from individual process timeout.");
        yield return Def("RULE_SOURCE_PROCESS_TIMEOUT", "Rule Source process timeout", "One source-control subprocess exceeded its individual configured bound.", "RuleSource", "Error", "RetrySafe", true, false, "Inspect the safe command category and timeout-scope diagnostics before retrying.");
        yield return Def("RULE_SOURCE_REF_NOT_FOUND", "Rule Source ref not found", "The requested Git discovery reference does not exist or could not be resolved by the selected source.", "RuleSource", "Error", "RetryAfterInput", true, false, "Correct the requested ref or select a different compatible source; do not increase timeouts.");
