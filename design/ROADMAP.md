...
- [ ] **FR-011 — GM/AI Context Assembly, Mandatory Read Discipline, and Gameplay Turn Persistence** — Define storage-aware, relevance-filtered context assembly from authoritative state and enforce automatic persistence for every state-changing gameplay turn. A turn is not complete until its Affected Set has been written to authoritative owners, validated, and read back where required; conversation context cannot substitute for canonical reads or writes, and normal play requires no player-issued save command.
