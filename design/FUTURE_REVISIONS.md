...
- **Priority:** High
- **Status reason:** Implemented through the portable Managed Operation contract, SQL Server-backed reference worker and migration 007, progressive preparation state connected to readiness and context retrieval, service diagnostics, approval and prerelease provenance changes, automated regressions, and explicit real-client acceptance scenarios. Corrective deployment maintenance subsequently made the pre-007 base control plane schema-safe, added database-independent configuration discovery, fail-safe semantic errors, automatic diagnostic fallback, bounded Error Dumps, and a genuine migrations-001-through-006 LocalDB upgrade regression. Live-acceptance maintenance then propagated cancellation to the durable owner, added operation-correlated diagnostics through migration 008, simplified Git acquisition to retain only manifest-defined rule payloads, made compatible user-selected sources first-class, and restored a separate full review archive. A later live power-loss regression added migration 009, renewable execution ownership, ownership-guarded writes, bounded orphan reconciliation, and same-identity recovery without creating another Future Revision.
...
- **Closure references:** [Managed Operations](../docs/persistence/MANAGED_OPERATIONS.md), [Managed Rule Publication](../docs/rules/MANAGED_RULE_PUBLICATION.md), [Errors and Portable Diagnostics](../docs/support/ERRORS_AND_PORTABLE_DIAGNOSTICS.md), [Release and Version Provenance](RELEASE_VERSIONING.md), [FR-021 Implementation Audit](audits/FR_021_DURABLE_MANAGED_OPERATIONS_AUDIT.md), [FR-021 Pre-Migration Control-Plane Repair Audit](audits/FR_021_PRE_MIGRATION_CONTROL_PLANE_REPAIR_AUDIT.md), [FR-021 Rule Source Live-Acceptance Repair Audit](audits/FR_021_RULE_SOURCE_LIVE_ACCEPTANCE_REPAIR_AUDIT.md), and [FR-021 Orphaned Managed Operation Recovery Audit](audits/FR_021_ORPHANED_MANAGED_OPERATION_RECOVERY_AUDIT.md)
